...
export default function AdminMembers() {
  ...
  const fetcher = () => clienteAxios('/api/infoUsuarios', {
    headers: {
      Authorization: `Bearer ${token}`
    }
  }).then(data => data.data)
  const { data, error, isLoading } = useSWR('/api/infoUsuarios', fetcher)

  if (isLoading) return 'Cargando...';

  //FUNCION PARA MOSTRAR A LOS USUARIOS
  const miembrosActuales = data;


  let results = [miembrosActuales]
      ...
      <div className="w-[968px] h-[812px]  ml-[50px] mt-[50px] rounded-[2em] bg-white overflow-x-auto">
        ...
        <div className="py-[40px] pl-[40px]   border-b-2 grid grid-cols-2 ">
          <div>
            ...
            <p className="mt-[10px]">Miembros</p>
          </div>
        </div>
        ...
        <table className="min-w-full text-left">
          <thead>
            <tr>
              ...
              <th className="border-b">Apellido</th>
          ...
          <tbody className="" >
            {miembrosActuales.map((row, index) => (
              <tr key={index} >
                <td className="border-b px-4 pb-[15px] pt-[15px] ">{row.name}</td>
                <td className="border-b ">{row.lastname}</td>
                <td className="border-b ">{row.address}</td>
                <td className="border-b ">{row.email}</td>
                <td className="border-b ">{row.statusSubscription}</td>
              </tr>
